...
void _darray_pop(void* array, void* dest){
    ...
    if(length == 0){
        KWARN("Tentativa de pop em DArray vazio");
        return;
    }
...
void* _darray_pop_at(void* array, u64 index, void* dest){
    ...
    if(index >= length){
        KWARN("Tentativa de remoção do Array com índice fora dos limites! Índice: %llu, Tamanho: %llu", index, length);
        return array;
    }
    ...
    // Se não for o ultimo elemento, "snip out the entry and copy the rest inward"
    if(index != length -1){
        kcopy_memory(
...
void* _darray_insert_at(void* array, u64 index, void* value_ptr){
    ...
    if(index > length){
        KWARN("Tentativa de inserção no DArray com índice fora dos limites! Índice: %llu, Tamanho: %llu", index, length);
        return array;
    }
    ...
    // Se não for o ultimo elemento, "make space" para o novo elemento
    if(index != length-1){
        kcopy_memory(
            (void*)(addr + ((index + 1) * stride)),
            (void*)(addr + (index * stride)),
